...
export function DeleteExpenseModal({ open, onOpenChange, onConfirm, expenseDescription }: Props) {
  return (
      ...
      <DialogContent className="max-w-[440px] p-0 overflow-hidden bg-[#0c0c0e] border-[#1c1c1f] shadow-2xl">
        <DialogHeader className="px-8 py-6 border-b border-[#1c1c1f]">
          <div className="flex items-center gap-3">
             ...
             <DialogTitle className="text-xl font-bold tracking-tight text-white">Excluir Lançamento?</DialogTitle>
          </div>
        </DialogHeader>
        ...
        <div className="p-8 space-y-6">
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground leading-relaxed">
              Você está prestes a excluir o lançamento {expenseDescription ? <span className="text-white font-bold">"{expenseDescription}"</span> : "selecionado"}.
            </p>
            <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/10 flex gap-3">
              <AlertTriangle className="h-5 w-5 text-amber-500 shrink-0" />
              <p className="text-xs text-amber-500/90 leading-relaxed font-medium">
                Este valor será removido do seu histórico financeiro e não será mais contabilizado. Esta ação não pode ser desfeita.
              </p>
            </div>
          </div>
